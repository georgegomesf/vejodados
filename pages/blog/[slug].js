    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const responsePost = token != false ?
                await axios.get(api + '/vejodados/wp-json/wp/v2/posts?status=private&slug=' + slug + '&_fields=slug,author,content,date,title,categories', { headers: { "Authorization": `Bearer ${token}` } })
                : await axios.get(api + '/vejodados/wp-json/wp/v2/posts?slug=' + slug + '&_fields=slug,author,content,date,title,categories')
            if (responsePost.status === 200) {
                setPost(responsePost.data)
                const responseCategorias = await axios.get(api + '/vejodados/wp-json/wp/v2/categories?_fields=id,name,slug&id=' + responsePost.data[0].categories[0])
